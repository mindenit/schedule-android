...
class FiltersBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lessonTypes = ChipGroup(requireContext()).apply {
    ...
    private fun loadData() {
        ...
        scope.launch {
            try {
                ...
                // Collect from cache (across all cached months), independent of hidden subjects
                ...
                teachersCached.forEach { (idT, names) -> teachersMap.putIfAbsent(idT, names.second.ifBlank { names.first }) }
                val teachersList = teachersMap.entries
                    .sortedBy { it.value.lowercase() }
                    .map { it.key to it.value }

                val auditoriumsMap = LinkedHashMap<Long, String>()
                auditoriumsApi.forEach { auditoriumsMap[it.id.toLong()] = it.name }
                auditoriumsCached.forEach { (aId, aName) -> auditoriumsMap.putIfAbsent(aId, aName) }
                val auditoriumsList = auditoriumsMap.entries
                    .sortedBy { it.value.lowercase() }
                    .map { it.key to it.value }
                ...
                val subjectsList = subjectsMap.entries
                    .sortedBy { it.value.lowercase() }
                    .map { it.key to it.value }
                ...
                // Silently ignore, leave lists empty
            } finally {
